...
let res0 = slice(input);
let data1 = slice(res0);

const part1Decode = s => stringToCharCodes(s)
        .map(n => (n ^ 0b01010101).toString(2).padStart(8, '0'))
        .map(s => rotateRight(s))
        .map(s => parseInt(s, 2))
        .toString()

let res1 = part1Decode(data1);
let data2 = slice(res1);

const part2Decode = s => stringToCharCodes(s)
        .map(n => {
...
const part3Decode = s => {
    let data = stringToCharCodes(s);
    let key = stringToCharCodes('==[ Layer 4/6: Network Traffic ]').map((n, i) => data[i] ^ n);
    return data.map((n, i) => n ^ key[i % 32]).toString();
}

let res3 = part3Decode(data3);
let data4 = slice(res3);

const part4Decode = data => ipv4.parse(data).filter(packet => {
    ...
    return true;
}).map(packet => packet.data.toString()).join('')
...
const part5Decode = (s) => {
    ...
    let kek = data.slice(0, 32);         // 32 bytes
    let iv1 = data.slice(32, 40);        // 8 bytes
    let encryptedKey = data.slice(40, 80); // 40 bytes
    let iv2 = data.slice(80, 96);        // 16 bytes
    let payload = data.slice(96);        // remaining bytes

    let key = aes.keyUnwrapRFC3394(kek, iv1, encryptedKey);
    let plaintextBytes = aes.ctrDecrypt(key, iv2, payload);
